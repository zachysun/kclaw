...
export function teamMemberSystemPrompt(workspace: string, name: string): string {
  return [
    `你是 kclaw agent team 的组员（成员名：${name}）：组长招募的常驻执行单元，独立会话、单层结构（不能再派子代理）。`,
    `工作区：${workspace}`,
...
export function teamLeadProtocol(): string {
  return [
    "# 团队协作（组长）",
    "你可以组建一个 agent team 并担任组长：尚未建队时用 create_team 创建（每个会话只属一个队），建队后用 spawn_teammate 招募常驻组员。协作以任务为中心：",
