...
> 真相源：`packages/core/src/hooks/types.ts`（位置与契约）、`hooks/positions.ts`（运行时清单）、`hooks/builtin.ts`（内置钩子单一真相 `BUILTIN_HOOK_SPECS`，管理面 `BUILTIN_HOOK_DEFINITIONS` 由它投影）。机制见 [hooks](../core/hooks.md)。
...
| `tool-before` | `{ toolCall }` | — | 观察 + 闸门（deny 档失败 = 该工具被拒绝） |
...
| `compaction-after` | `{ phase, result }` | — | 观察位：压缩结局 |
| `think-after` | `{ block }` | — | 观察位：思考块完成 |
| `system-before` | `{ base }` | `string[]` | 追加位：提示词段落（多钩子累积） |
...
| `skill-wrap` | llm-before | 10 | skip | 技能与 @ 文件点名的隐式包装（只改模型视图） |
...
| `mid-run-panic` | compaction-check | 10 | skip | 红线水位的迭代边界压缩判定（不限时） |
...
| `post-run-compaction` | run-after | 20 | skip | 黄线水位的收尾压缩（不限时） |
| `follow-check` | run-after | 30 | skip | 排一个记忆空闲检查 |
| `system-materials` | system-before | 10 | skip | 收集认知与技能清单两个提示词段（live 段） |

`compaction-after` 位置没有静态内置条目：压缩钩子经 `compactionAfter` 回调把结局（applied→ok / failed / cancelled，declined 静默）转发到该位置的链上。

子代理 run（`childRun`）的派生跳过：`memory-inject` 不检索、`autoname` 跳过、`follow-check` 不挂、`system-materials` 返回空段。

## HookMeta 字段

用户钩子文件与内置钩子共用同一份元数据形状：
...
| `order` | number | 同位置内升序执行；用户钩子默认 1000（落在内置 10–99 之后） |
| `failure` | `"fatal" \| "skip" \| "deny"` | 失败策略（见下） |
| `origin` | `"builtin" \| "user"` | 来源 |
| `timeoutMs?` | number | 条目级超时覆盖；缺省 = 链默认（config `hooks.timeoutMs`，5000ms）；Infinity = 不限时 |
| `error?` | string | 装载失败原因（用户钩子；失败条目不运行） |

## failure（3 档）

| 值 | 含义 |
|----|------|
| `skip` | 失败即跳过，发一条 `hook.failed`，run 照常（用户钩子默认） |
| `deny` | 失败否决所在闸门（tool-before 上 = 该工具不执行，fail-closed 自选档） |
