...
| readonly 权限模式 | 删全部 sensitive 工具（`exec` / `fs_write` / `fs_edit`）——可见范围缩小，权限判定仍是边界 |

团队工具的注册条件按身份分化：`lead` 身份（含未建队的主线会话——"预备组长"，见 [agent-team](../core/agent-team.md)）注册全部七个；`member` 身份注册 `send_message` / `list_agents` / `task_*` 五个（不含建队/添加）；不在团队里的会话（job 会话、非组员的 subagent 会话）一个都没有。
