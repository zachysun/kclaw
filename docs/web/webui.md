...
| 技能（tab） | `skills/SkillsView.tsx` | 只读技能页：左栏技能清单（名字 / 作用域 / 可见性标记 / 描述，`GET /skills?workdir=`，workdir 取当前所选会话的工作目录）、右栏点开后的 `SKILL.md` 正文（`GET /skills/:name?workdir=`）。技能是文件即真相（`~/.kclaw/skills/` 与工作区 `.kclaw/skills/`），本页没有编辑动作；`user-invocable: false` 的技能服务端直接 404、不显示。空清单提示把技能目录放进两个位置之一、下一个会话轮次即生效。机制见 [skills](../core/skills.md) |
