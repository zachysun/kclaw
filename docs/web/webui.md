...
| 审计（tab） | `audit/AuditView.tsx` | 会话下拉 + `GET /sessions/:id/events` **单源**拉取该会话完整事件流，按**流序**（append-only，旧在上新在下，像日志）渲染四类行：`message` 事件按块摊平成"每块一行"（点击展开完整块，工具行按 callId 关联 tool 消息的 `grantedBy` 显示放行原因）、`compaction` 事件渲染成"压缩"行（触发方式〔自动（收尾）/自动（运行中）/手动，手动附 focus；`emergency` 急救加"·超限急救"标注〕、被压范围 `from–upto`、条数，点击展开段摘要与总摘要全文）、`memory` 事件渲染成"记忆"行、`system` 事件渲染成"系统提示词"行（折叠给空白折叠后的开头 60 字片段 + 总字符数，点击展开完整提示词全文；与相邻上一条"系统提示词"行文本不同时标"已变化"）；会话元数据事件（session.created/renamed/…）跳过。纯只读、无独立 `/audit` 路由 |
...
- **消息轨迹没有独立 /audit 路由**：轨迹的唯一事实来源是该会话的**事件流**——审计页选中会话后拉 `GET /sessions/:id/events` 单源渲染（四类行：message 块 / compaction"压缩"行 / memory"记忆"行 / system"系统提示词"行，按流序，见上文审计 tab）；`GET /sessions/:id/compactions` 是事件流里 `compaction` 事件的只读投影视图，无记录时不显示。
