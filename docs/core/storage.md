...
| `permissions.sessionGrants` | `true` | 会话内"本次允许"记忆是否生效（run 级，见 [permissions](./permissions.md) 第 11 节） |
| `permissions.autoLearnThreshold` | `3` | auto 模式的归纳阈值：同一操作被连续 `once` 批准多少次后自动沉淀为项目档规则；`0` 关闭归纳（仅保留 auto 模式的判定链） |
| `permissions.defaultMode` | `"default"` | 新会话的初始权限模式（daemon 创建的新会话——HTTP `POST /sessions` 与任务调度建会话——创建时固化为 `meta.mode`，改它只影响之后新建的会话）；非法值回落 `"default"` 并告警 |
...
- `events.jsonl`：**唯一真相**，append-only 事件流，一行一个 `SessionEvent`（JSON 序列化）。共 10 种事件：会话生命周期 `session.created`（含创建时固化的初始权限模式 `mode`；子代理会话还带 `parentSessionId`）/ `session.renamed` / `session.deleted` / `session.restored` / `session.set`（model / mode / disposition 的会话级设置；旧 `readonly` 布尔字段是 legacy，读取时映射为 mode），外加内容类 `message`（一条消息）、`compaction`（一次压缩审计）、`memory`（一次记忆落盘审计）、`system`（一条系统提示词审计，每次对话运行落一条拼装完成的全文）、`sandbox.checked`（一条沙箱状态审计，每次对话运行探测后落一条 `{enabled, available, unavailableReason?}`）。所有写入都先落事件，再把事件折进 meta.json 投影（见下）。
- `meta.json`：**派生投影**（`SessionMeta`），由事件流经 `applyEvent` 逐条折叠得出；meta.json 缺失或损坏时 `meta()` 自动从事件流重建（`rebuildMeta`），任何时候删掉它也能重建。崩溃恢复时允许它滞后于事件流（meta 只是投影、非真相，不会丢数据）；滞后不会被后续写入自动追平——`appendEvent` 先读当前投影、只折入新事件——仅在 meta.json 缺失或损坏时经 `rebuildMeta` 重放整条事件流整流。整文件原子重写（`updateMeta` 合并 patch、`undefined` 键删除；`message` / `compaction` 事件会推进投影的 `updatedAt`，`memory` / `system` / `sandbox.checked` 事件不推进——审计类事件不算会话「更新」）。两条特殊的投影效果：`system` 事件会把全文 upsert 进投影的 `systemBaseline`（提示词缓存纪律的冻结基线，见下），`compaction` 事件会把 `systemBaseline` 清除（压缩改写消息历史 = 缓存必然冷启动，正是重冻结边界）。
...
`meta.json` 字段：`SessionMeta { id, title, createdAt, updatedAt, jobId?, workdir?, model?, mode?, parentSessionId?, deleted?, deletedAt?, compactedSummary?, compactedUpto?, compaction?, dispositionOverride?, systemBaseline? }`，其中 `model` 为会话级模型覆盖（空/缺省回落 daemon 默认）、`mode` 为会话权限模式（`"readonly" | "default" | "acceptEdits" | "trusted" | "auto"` 五档，缺省 default；旧 `readonly` 布尔读出时映射为 `mode:"readonly"` 并删除布尔键，写入端只产 `mode`，见 [permissions](./permissions.md)）、`parentSessionId` 为子代理会话的父会话标识（普通会话缺省；引擎侧一切子代理特化从它派生，见 [subagents](./subagents.md)）；`compaction` 是分层压缩状态 `{ segments, top, upto }`（语义见 [compaction](./compaction.md)），由 `compaction` 事件投影（每次压缩把新段折进 `segments`，`updateMeta` 不再直接合并它）；`compactedSummary`/`compactedUpto` 是 v1 压缩的遗留字段——不再被清除，但运行侧读压缩视图时 `compaction` 优先（压缩引擎 `Compactor` 的 `compact` 里 `prev` 先读 `compaction`，见 [compaction](./compaction.md)），两者并存无功能影响；`dispositionOverride` 是会话级处置覆盖（`"steer" | "wait" | "interrupt"`，优先于 `sessions.defaultDisposition`；服务端路由仍接受三值写入，但客户端当前只写 steer/wait——interrupt 在 Web 与 CLI 都是一次性动作、不落覆盖，`"interrupt"` 值只会来自历史遗留，见 [webui](../web/webui.md) 与 [run-manager](../server/run-manager.md)）；`systemBaseline` 是冻结的系统提示词基线 `{ text, frozenAt }`（提示词缓存纪律：非空时 run 直接以其为系统提示词、组装链整体跳过，认知/技能清单/AGENTS.md/钩子段落的变化留到下一次压缩清除基线后随首个 `system` 事件重新固化；投影上 `system` 事件 upsert（文本未变保留 `frozenAt`——它记录这份文本成为基线的时刻）、`compaction` 事件清除，不推进 `updatedAt`）。
