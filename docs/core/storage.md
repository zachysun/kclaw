...
| `sessions.contextTokens` / `compactAtRatio` / `compactPanicRatio` / `compactTargetRatio` / `toolResultKeep` | `128000` / `0.66` / `0.85` / `0.33` / `8` | 上下文压缩（见 [compaction](./compaction.md)）：token 预算、黄线（估算发送量达预算 × 0.66 即触发压缩）、红线（运行中占用达预算 × 0.85 时在迭代边界触发中途压缩）、压缩后保留部分的目标比例（预算 × 0.33）、发送时保留原文的最近工具结果条数。五个字段均可选，缺省值在读取处补齐（前四个在 run 装配 core `executeRun`，`compactTargetRatio` 在压缩引擎 `Compactor`） |
