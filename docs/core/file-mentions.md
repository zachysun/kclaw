...
在用户消息里用 `@路径` 引用项目文件（如「把 @src/main.ts 的逻辑讲给我听」），让模型在回答前先读取该文件的完整内容。实现分三层：纯函数层（core `packages/core/src/mentions.ts`，负责提取、候选补全与模型侧包装文本，无 Node API，浏览器构建可直接引用）、运行组装层（core `run-assembly.ts` 的 `resolveFileMentions`，把 token 解析成工作区里的真实文件）、数据源层（server `GET /fs/files`——提供抽屉的候选清单）。WebUI 的输入框用同一套纯函数做联想抽屉；手打与抽屉补全的引用走完全相同的解析路径，行为一致。

## 设计决策

- **只改模型看到的输入，持久化与气泡保持原文**：与技能指定同一原则（见 [skills](./skills.md)）——`message` 事件、events.jsonl 与聊天气泡存的是用户输入的原文；只有发给 provider 的那份文本追加了读取指示。用户看到什么、发的是什么，系统里就存什么。
...
- **只有用户消息参与指定检测**：`trigger:"user"` 才做包装；job 提示是 daemon 生成的内部指令，不参与指定检测（与技能指定同一开关）。
...
- **`extractFileMentions(text)`**：按边界规则扫描全文，把每个 `@` 到下一个空白之间的 token 提取出来；重复 token 合并、扫描顺序保留。只做语法——存在性与工作区边界在运行组装层判断（那里才知道工作区在哪）。
...
- **`wrapFileMentions(text, resolutions)`**：生成追加在用户消息后面的指示行。一条引用生成「请用 `fs_read` 工具读取该文件的完整内容，再结合它处理本条消息」；多条逐条列出；`missing` 的生成「引用的文件不存在或已删除：请向用户说明，不要尝试读取」。没有需要指示的内容时返回 `undefined`（原样发送）。

### 运行组装（`run-assembly.ts` 的 `executeRun`）

1. **工作区解析** `resolveFileMentions(text, workspace)`：每个 token 相对会话工作目录（默认 daemon 的 `workspace`）解析，realpath 后必须落在工作区内且是普通文件 → `ok`；不是文件 → `missing`；逃逸 → 整个丢弃（不产生指示行）。
2. **合并** `combineMentionTexts`：技能指定的包装文本（见 [skills](./skills.md)）与文件指定的指示行合并成一份 `llmUserText`——两者都不匹配时保持原样。
3. **应用**：`llmUserText` 经内置 `skill-wrap` hook（`llm-before` 位置，见 [hooks](./hooks.md)）的 `withLastUserText` 替换发送消息里最后一条 user 消息的文本。只在这一步影响模型输入。

## 数据源：GET /fs/files（server `routes/fs.ts`）

抽屉的候选清单来自 `GET /fs/files?workdir=<绝对路径>`（默认取 `config.workspace`），响应 `{workdir, files, truncated}`——`files` 是工作区相对路径（POSIX 分隔、仅文件、大小写不敏感排序），`truncated` 是清单是否在 5000 条处被截断。鉴权与其他 API 路由相同（Bearer）。
...
- [skills](./skills.md)：同机制的先例——指定包装、`skill-wrap` 内置 hook 与 `withLastUserText`
- [hooks](./hooks.md)：`llm-before` 位置与 `skill-wrap` hook（文件与技能指定共用）
...
- [protocol](./protocol.md)：message 事件附带的原文（包装不改写持久化）
