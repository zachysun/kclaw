...
`packages/core/src/client-http.ts` 是所有 daemon 客户端（CLI 的 `KclawClient`、WebUI 的 `api.ts`）共用的 HTTP 请求基座，经 `@kclaw/core/client-http` 子路径出口对外发布。它把每处客户端都要重复写的同一套请求管道收敛成一份：Bearer 注入、JSON body 序列化、非 2xx 的错误提取、401 钩子、204/空响应解析。

实现只依赖浏览器/Node 都有的全局 `fetch`，**不 import 任何 `node:*` 模块**——WebUI 把它打进浏览器产物时不会把 Node 绑定的代码带进去（与 `@kclaw/core/protocol` 同一先例）。

## 设计决策

- **一个错误路径，两种消息来源**：非 2xx 时优先取服务端 `body.error` 字符串（服务端统一错误形状就是 `{error: string}`，见 [http-api](../server/http-api.md)），取不到（非 JSON 错误体等）退回 `HTTP <status>`。抛出的 `HttpRequestError` 始终携带 `status`，调用方可以按状态码分支而不必解析消息文本。
- **401 用钩子而不是抛错语义**：每次 401 响应都在抛错之前触发一次 `onUnauthorized`（比如 WebUI 的"清 token 回输入页"），然后照常抛错——调用方不用在 catch 里猜状态码做重入。
...
- **WebUI**（`packages/web/src/api.ts`）：`createApi` 的 `get/post/patch/del/upload` 全部经 `httpRequest`。URL 解析（空 base = 同源，daemon 自己托管 SPA）与 401 重入钩子（App 清除 token、回 token 输入页）是 web 本地逻辑，留在 `api.ts` 里；`ApiError` 现在就是 `HttpRequestError` 的别名导出（同一个构造函数，`instanceof` 判断不变）。
