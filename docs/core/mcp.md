...
- **配置管理独立于手写的配置文件，分全局与项目两层**：WebUI 增删改的配置写进 daemon 主目录的 `mcp.json`（JSON 格式，与常见 MCP 客户端的习惯一致），不动 `config.json`。项目层配置写进工作区的 `.kclaw/mcp.json`（见下"项目层"），与全局层按名字合并。
...
- **工具名加前缀，避免冲突**：来自 MCP 的工具统一命名为 `mcp__<server>__<tool>`（例如 `mcp__filesystem__read_file`），不同 server 的同名工具、以及与内置工具之间靠前缀天然分开。万一仍与内置工具撞名，适配器的实现覆盖内置的那个，并打一行日志说明。经 API 新增的 server 名字限定为字母、数字、下划线和连字符（名字会进模型可见的工具名）。
...
**全局层**的配置文件是 daemon 主目录下的 `mcp.json`（权限 0600，原子写）：
...
`enabled === false` 的条目停留在 `"disabled"` 状态，永远不会发起连接。读写函数收在 core 的 `storage/mcp-config.ts`：`loadMcpJson` / `saveMcpJson`（全局层 `mcp.json` 的读与写）、`loadProjectMcpServers` / `saveProjectMcpJson`（项目层的读与写，含 git 跟踪检测与 gitignore 防御）。
...
export class McpManager {
  constructor(opts: {
    ...
    // global = ~/.kclaw/mcp.json；project = <workspace>/.kclaw/mcp.json
    ...
    //（含被另一层遮蔽的条目——persist 是整层写入，丢一个遮蔽条目就再也回不来）。
    // daemon 把 global 接到 mcp.json、project 接到项目文件；manager 本身不管存储。
  ...
  // 新增进指定层（默认 global）；重名（两层中任一占用）/空名抛错，错误指明占用层；后台连接
...
- **reconcile 以文件为准，永不持久化**：新出现的项目条目 → 连接它（若全局层有同名条目，先停掉旧连接再按项目条目重建，遮蔽生效）；配置变了的项目条目 → 先断开再重连；消失的项目条目 → 从全局层恢复（配置与全局相同则只翻转 `scope`、连接不动；不同则重连），全局层没有同名条目就直接移除。
...
- daemon（`packages/server/src/daemon.ts`）：恒定构建管理器（配置来自两层读，`persist` 回调按层分发——global 接到 `saveMcpJson`，project 接到 `saveProjectMcpJson`（并补挂项目 watch））；组装时挂上项目文件 watch；启动监听之后用 `void mcpManager.start()` 触发连接、不等它完成就开始对外服务，晚连上的 server 从下一轮 run 起可用；停止序列中先关 watch（不遗留挂着的防抖回调）再关管理器。RunManager 把上面提到的 `extraTools` 函数作为依赖传给 core `executeRun`，由后者在每个 run 求值并注入这些工具（见 [run-manager](../server/run-manager.md)）。
- HTTP 接口（见 [http-api](../server/http-api.md)）：`GET /mcp` 返回 `{servers: status()}`（没组装管理器时是空列表；每条带 `scope`）；`POST /mcp/servers`（新增，可选 `layer` 字段 `"global" | "project"`、默认 global）、`PATCH|DELETE /mcp/servers/:name`（改/删，作用于生效条目、持久化进拥有它的那层文件）、`POST /mcp/servers/:name/enable`（启停）、`POST /mcp/servers/:name/reconnect`（重连）是 WebUI MCP 栏的管理动作，任何保存动作都会触发一次对应层的持久化。
