...
`packages/core/src/provider/` 把"调用一个大语言模型"收敛为一个最小接口 `LlmClient`：输入一次对话请求，输出一串流式事件。它只做四件事——把内部请求转成 OpenAI 兼容的 `/chat/completions` 调用、解析流式响应、给整个请求加超时、对临时性失败做带退避的重试。不读配置、不感知进程模型：baseUrl/apiKey/model 怎么来是 daemon 侧的事（见下文"端点解析"）。

## 设计决策

- **只认一种线上协议**：任何提供 OpenAI 兼容 `/chat/completions` 端点的服务（OpenAI、各类中转、本地模型网关）都无需单独适配；差异全部留在 baseUrl 指向的端点上。
- **core 与传输解耦**：`agent/loop.ts` 只依赖 `LlmClient` 这个 async iterable 接口，不知道 fetch、SSE（Server-Sent Events：服务器通过 HTTP 持续推送文本行的流式格式）的存在；测试注入假 client 即可跑通整个循环。
- **超时覆盖整个请求**：`AbortSignal.timeout` 同时约束"等响应头"和"读流式响应体"两个阶段——卡住的 provider 流（无响应头、或响应体中途停住）不可能让一个 run 永久停在那里。
...
- **重试藏在 client 内部，不放在循环里**：`runAgent` 调一次 `stream()` 就是完整的一次"可能含内部重试"的调用；循环层再重试就是双重重试。循环通过 `onRetry` 钩子感知重试（转成 `llm.failed {willRetry:true}` 事件）。
- **已产出事件绝不重试**：流已经吐出过事件就说明消费方可能收到过，重试会造成输出重复——此时错误直接抛出。
...
- 请求体固定 `stream: true` 与 `stream_options: {include_usage: true}`（最后一个 chunk 会带 token 用量）；URL 为 `baseUrl` 去掉一个尾部 `/` 后拼 `/chat/completions`；鉴权用 `Authorization: Bearer <apiKey>` 头（Bearer 是 HTTP 标准的"令牌跟在 Bearer 后面"格式）。
...
undefined、null、`""` 三种"没有内容"统一跳过——这保证"完全没有参数流的调用"（arguments 从未出现）也能存活，循环层把它按 `{}` 解析。
...
非 2xx 且响应体读成功时抛 `llm http <status>: <body文本>`——错误体也是被超时约束的，不会把一个挂起换成另一个挂起。
...
- CLI 首跑判定（`packages/cli/src/provider-check.ts` 的 `detectProviderStatus`）用同一套优先级输出三态：`config`（default 指向存在的条目）/ `env`（任一 `KCLAW_LLM_*` 非空）/ `missing`。
...
- **流中断在工具调用中间**：已发过 `tool_call_started` 但流死掉 → 循环为悬空调用合成 error result，保证历史里 tool_call 永远有配对结果（OpenAI 兼容 API 对无配对 tool_call 的下轮请求回 400）。
- **错误体读失败的双重保底处理**：超时后的读失败按超时报告；超时前的读失败吞掉以保住状态行分类（见上表）。
