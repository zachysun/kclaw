...
- **后台模式（`run_in_background: true`）**：派发立即返回子会话 id 与取回提示（不阻塞父 run），子代理的**生命周期挂到父会话**而不是父 run——父 run 结束或中止的信号刻意不递给 spawner，后台子代理照常跑完；删除父会话则先取消它仍在跑的后台子代理，再走既有的级联软删。子代理落定时主会话收到一条完成通知（一条带 `kind:"system"` note 的 assistant 消息：标签、截断的结果摘要、`subagent_collect` 取回提示——它**不触发新 run**，下一轮对话自然看到它；父会话已删则静默跳过）。完整答复用 `subagent_collect {childSessionId}` 按需取回（只能取本会话派出的子代理）。后台任务有独立的每父会话并发上限（`subagents.maxBackground`，与阻塞档分开计数）。
...
**后台派发的生命周期差异**：`run_in_background: true` 时上面第 2 步的并发上限换成后台档（`maxBackground`），第 3 步后立即返回子会话 id（工具结果是 `已在后台派出子代理「…」（会话 …）…完成后这里会收到一条完成通知，届时可用 subagent_collect …`），不等待子 run 落定；子 run 完成后主会话收到完成通知消息（不触发 run），删父会话时 `cancelBackgroundForParent` 先取消在跑的后台子代理。
...
状态行只属于阻塞派发：后台子代理没有等待中的工具结果可流，状态行静默（进度可见性由完成通知承担）；但子代理的**确认卡与问题卡**在两种模式下都转发到父频道——敏感操作不能因为后台了就不问人。
