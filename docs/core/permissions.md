...
- **配置**（`config.yaml` 的 `sandbox:` 节，daemon 级，默认开）：
  ...
  - **审计（批次 D）**：run 装配在每次探测后向会话事件流落一条 `sandbox.checked` 审计事件（第 10 种会话事件，字段 `enabled`=config 开关 / `available`=探测结果 / `unavailableReason`=原因；主动关闭沙箱时 `available` 恒 false 且不带原因）。每 run 恰一条，只落盘不上总线、不进 meta 投影、不推进 updatedAt，写失败即 run 失败（与 `system` 审计事件同契约）——轨迹页可逐 run 回看"当时沙箱是什么状态"，配合 grantedBy / deny note 串成完整审计链（见 [webui](./webui.md)）。
