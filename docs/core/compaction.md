...
各时机与在飞压缩的关系遵循一条不变量：**任何时刻同一会话至多一个压缩在跑**。后台预压（`compaction-check` 位置的内置钩子 `background-precompact`，见 [hooks](./hooks.md)）只在无在飞、无挂起成果时开工；中途压缩（`mid-run-panic`）遇在飞后台时等它结束再决定（结束后若水位已落回红线以下，就只应用后台成果、不再压缩）；收尾压缩（`post-run-compaction`，收尾路径里 await）遇在飞后台同样先等。超限急救（`overflow-rescue` 位置的 `overflow-emergency`）是唯一的例外——救援路径等不得，先掐掉在飞后台再立即同步急救，被掐掉的成果丢弃（原文无损，下次重压）。中途压缩与收尾压缩成功后的新压缩视图从下一次请求开始生效；超限急救成功后整次请求静默重发一次。
...
| 场景 | 行为 |
|------|------|
| 同步压缩开工时已有挂起视图 | 丢弃挂起（作废计数）：同步结果基于的元数据已含后台成果，再应用旧视图等于把压缩视图回滚 |
| 中途压缩遇到在飞后台 | 等它落定，重估水位：过红线就正常压，没过就只应用挂起成果 |
| 收尾压缩遇到在飞后台 | 等它落定，再按黄线正常判断 |
