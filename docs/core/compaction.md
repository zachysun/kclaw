...
每轮实际发给模型的内容 = 脉络项（一条承载总摘要的 system 消息） + m4–m7 的原文
```
...
发送消息永远不等待压缩——没有"发送前预压缩"这个环节，回车后立即开始运行。压缩发生在四个互不重叠的时机，前三个自动、最后一个手动：

| 触发点 | 时机与条件 | phase（事件与内部标记） | 审计 trigger |
|--------|-----------|------------------------|--------------|
| 收尾压缩 | `runAgent` 返回之后：run 以非 aborted/error 终态结束且水位 ≥ 黄线 | `post-run` | `auto` |
| 中途压缩 | 迭代边界（每轮工具批次完成后、下一次模型调用前）：水位 ≥ 红线 | `in-run` | `in-run` |
...
历史会话里由旧机制写入的 `kind:"compact"` note 块（当时把总摘要挂在用户消息上、每轮重挂）不再由服务端生成，但已持久化的照旧渲染：WebUI 仍按它折叠出上下文条（展示约定见 [webui](../web/webui.md)），CLI 对带结构化 meta 的旧 note 保持静默——旧会话的显示不受影响。
...
替换只发生在发给供应商的请求数据里，会话 JSONL 一个字不动。它不会产生无效请求：工具调用和结果消息的配对关系没变，只是结果消息**里面的文本**换短了。运行中途的保护由此自动获得：agent 循环每次请求都要过这个函数，一次运行里连续几十次工具调用时，较早的输出自动被省略，上下文不会随调用次数线性膨胀；预算取黄线值意味着"省略后的请求大小以触发线为界"——水位逼近黄线时，请求里的工具输出部分已经被压到只剩最近的几条。
...
- **会话正在运行时拒绝**：返回"会话正在运行"（HTTP 409）——压缩要读全量历史、写会话元数据，和运行中的写入并发会互相破坏。收尾压缩也算在这个"正在运行"窗口内：它发生在运行结束路径上、活动登记清除之前，期间手动压缩同样被拒。
