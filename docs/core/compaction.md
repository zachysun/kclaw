...
4. **后台预压缩**（压缩引擎的 `Compactor.background`）：水位进入预压区间就派一次后台压缩——前台继续运行不等它，摘要在后台生成、写入元数据后挂起，等下一次迭代边界再应用，把"等压缩"的等待从关键路径上挪走。
...
- **同一会话至多一个压缩在跑**：后台预压与同步压缩（中途/收尾/急救/手动）共用一个在飞登记表，同步路径遇在飞后台时等它落定再重新估水位做决定，而不是并发两个摘要调用互相踩写。等来的后台成果不浪费：直接应用，或作为同步压缩的兜底（等完发现水位已落回线下，就用后台成果、不再压一次）。
...
| 水位 | 对"当前会话全部未压缩历史"的 token 估算值（`estimateContextTokens`），是各道触发线的读数来源。 |
...
| 后台压缩 | 预压区间内启动的非阻塞压缩：前台继续运行，摘要在后台生成，完成后写入元数据并挂起待应用（见"机制四"）。 |
...
|--------|-----------|------------------------|--------------|--------|
| 后台预压 | 迭代边界：水位进入 [预压线, 红线) 区间且无在飞压缩、无挂起成果、无取消标记 | `in-run` | `in-run`（审计页显示"自动（运行中）"） | 不阻塞（后台进行） |
| 中途压缩 | 迭代边界：水位 ≥ 红线 | `in-run` | `in-run` | 挡下一次模型调用 |
| 收尾压缩 | `runAgent` 返回之后：run 以非 aborted/error 终态结束且水位 ≥ 黄线 | `post-run` | `auto` | 挡下一条出队消息 |
| 超限紧急急救 | 一次流式调用抛出上下文超限错误且零输出：不看水位线，"已经爆了"就是事实 | `in-run` | `in-run` 加 `emergency: true` | 阻塞（救援） |
| 手动 /compact | 用户主动调用，跳过一切触发线 | `manual` | `manual` | 忙时挂起（见"手动压缩"） |

水位是 `estimateContextTokens(会话历史)`：锚定最后一条助手消息记录的真实 `usage.inputTokens`（上一次实际发出的请求大小），锚之后的新消息按字符粗算。锚定使已被压缩的旧内容天然不计入——它们根本不在上一次请求里——所以直接对全量历史读数即可，不需要先切出未压缩部分。没有锚点的窗口（新会话首条、压缩后的首请求）由显式的固定开销项补上：run 装配在系统提示词定稿后按字符粗算一次"系统提示词 + 工具定义"的估算值，水位读数在无锚点时加上这一项（有锚点时不加——锚点里的 `inputTokens` 已包含它，再加就重复计）。

各时机与在飞压缩的关系遵循一条不变量：**任何时刻同一会话至多一个压缩在跑**。后台预压（`compaction-check` 位置的内置钩子 `background-precompact`，见 [hooks](./hooks.md)）只在无在飞、无挂起成果时开工；中途压缩（`mid-run-panic`）遇在飞后台时等它结束，再按挂起分界重估活跃段——仍超红线才同步压，否则直接应用后台成果；收尾压缩（`post-run-compaction`，收尾路径里 await）遇在飞后台同样先等（循环已结束，挂起视图取走丢弃——元数据已携带同一成果）。超限急救（`overflow-rescue` 位置的 `overflow-emergency`）是唯一的例外——救援路径等不得，先掐掉在飞后台再立即同步急救，被掐掉的成果丢弃（原文无损，下次重压；掐掉走 `abortInFlight` 不写取消标记——`cancel` 的标记会压制紧随其后的急救本身）。中途压缩与收尾压缩成功后的新压缩视图从下一次请求开始生效；超限急救成功后整次请求静默重发一次。
...
五条线的分工（默认值）：省略线（`compactPackRatio`，0.70）在每次构造请求时免费削掉装不下的旧工具输出；预压线（`compactAheadRatio`，0.75）到线就派后台压缩，前台无感；黄线（`compactAtRatio`，0.80）配收尾压缩——一次压缩就把保留部分压到预算的三分之一（`compactTargetRatio` 默认 0.33），余量已够；红线（`compactPanicRatio`，0.90）配中途压缩，防运行内部工具输出累积把水位推向预算，也是后台压缩的触发上限（水位已过红线就直接同步压，不再绕后台）。压完留 33%，意味着之后要再积累约 1.27 倍预算的新对话才到预压线（压缩要花两次模型调用，不能太频繁）；运行内更细粒度的膨胀由机制三在每次构造请求时按省略线自动省略，红线是它之上的保险。

自动压缩可以取消：WebUI 压缩指示行的"取消"按钮发 WS 帧 `compaction.cancel`，服务端中止在飞的摘要调用（含后台预压），并写一个会话级取消标记——本次运行内后续的自动压缩（中途、收尾、后台预压，以及超限急救——它走自动压缩的共用装配入口 `Compactor.auto`，标记在时同样不开工）都被这个标记压制，下一次运行开始时标记清除、恢复正常压缩（服务端细节见 [run-manager](../server/run-manager.md) 的 `cancelCompaction`）。运行被用户中止时在飞的后台压缩**不**随之中止：它的成果照常写入元数据，下次运行直接受益；daemon 退出时它被掐断，等于没发生。手动 /compact 不检查取消标记、与自动压缩互不干涉，WebUI 对手动阶段也不渲染取消按钮（`cancelCompaction` 不作用于手动压缩）。
...
同步压缩的代价是等待：压缩要花两次模型调用、数秒时间，收尾压缩挡住下一条出队消息，中途压缩挡住下一次模型调用。后台预压缩把这次等待从关键路径上挪走——水位刚过预压线（75%）就开始在后台做摘要，等水位爬到黄线时大概率已经做完，同步路径一看水位已落回线下就什么都不用做。整个机制由三块组成：

**触发**（`compaction-check` 位置的内置钩子 `background-precompact`，排在 `mid-run-panic` 之前）：每个迭代边界检查一次，条件全部满足才开工——水位落在 [预压线, 红线) 区间；该会话无在飞压缩、无挂起成果、无取消标记；运行未被中止。水位过红线时不走后台——那是同步中途压缩的地盘（同步路径当场就能挡住下一次请求，后台救不了近火）。

**执行**（`Compactor.background`）：登记在飞（与同步路径同一张登记表，"同一会话至多一个压缩在跑"由此保证）后立即返回，摘要在后台进行。两个刻意的设计：

- **不挂 run 的中止信号**：用户中途取消运行时，在飞的后台压缩照常跑完，成果照常写入元数据，下次运行直接受益；只有 daemon 退出会掐断它，等于没发生。
- **豁免黄线细判**（`background: true`）：普通自动压缩开工前会按黄线复核水位，预压区间本来就在黄线之下，这条复核会把后台压缩全部拦死，所以后台路径跳过它。

压缩本体与同步路径完全同一条代码路径（机制二的两次摘要调用、审计、事件一个不少）：`phase` 记 `in-run`、审计 trigger 记 `in-run`（审计页显示"自动（运行中）"），`compaction.started`/`completed` 照发——WebUI 的压缩指示行对后台压缩同样出现，取消按钮同样可用（`cancelCompaction` 掐的就是登记表里那个在飞任务）。

**完成与生效（双重交接）**：后台压缩完成时先把成果无条件写入——`compaction` 事件、`meta.compaction` 投影、总线事件全部落定；随后把新视图**挂起**在内存里（`takeParked` 可取）。应用发生在下一次迭代边界：`mid-run-panic` 在取消标记与中止信号都没命中后取挂起视图，有就直接采用为新压缩视图（挂起成立说明水位刚过预压线时压的，现在大概率已落回线下，不再压第二次）——当前运行的下一次请求立刻变小；取消标记压着本次运行时不应用，挂起留在原地，由下一次运行取用。挂起视图只在内存：daemon 重启会丢掉挂起本身，但元数据已经落盘，下次运行从 meta 读到同样的压缩视图，无损失。

与各同步路径的相处规则（详见"何时触发"一节的不变量）：

| 场景 | 行为 |
|------|------|
| 同步压缩**成功**时已有挂起视图 | 丢弃挂起：同步结果基于的元数据已含后台成果，再应用旧视图等于把压缩视图回滚。只在成功时作废——同步压缩没写成（水位/边界拦下、摘要失败）什么都没写，挂起成果仍是最新视图，兜底照常可用 |
| 中途压缩遇到在飞后台 | 等它落定，按挂起分界重估活跃段（重估读数受旧锚点影响偏高，安全方向）：仍超红线才同步压（活跃段切不出边界就放弃硬压），否则直接应用挂起成果 |
| 收尾压缩遇到在飞后台 | 等它落定（挂起视图取走丢弃——循环已结束，元数据已携带同一成果），再按黄线正常判断 |
| 超限急救遇到在飞后台 | 等不得：先掐掉在飞再立即急救，被掐的成果丢弃（原文无损，下次重压） |
...
- **会话正在运行时挂起，不再拒绝**：run 活动期间收到 /compact，返回 `{ queued: true, message: "已排队：当前运行结束后自动压缩" }`（HTTP 200），请求挂起在内存里；运行结束的收尾链在自动收尾压缩**之前**先冲刷挂起的手动压缩（内置钩子 `manual-compact-flush`），用户拿到的就是自己点的那次压缩，而不是先被自动压缩捷足先登。挂起期间再次 /compact 以后来的 focus 覆盖先前的（用户要的永远是最后一次指示）；挂起只在内存，daemon 重启即丢。挂起的手动压缩跳过忙碌/排队拒绝检查——它本来就在"运行结束后"这个时点执行，此刻队列里有消息是常态而非异常。
...
| 后台预压的摘要调用失败 | 记 `kclaw compaction (background) failed:` 日志 + 发 `completed {result:"failed"}`；失败等于没发生，挂起不产生，下个迭代边界可重试 |
| 中途压缩被用户取消 | 发 `completed {result:"cancelled"}`，不写压缩事件/投影；本次运行内不再自动触发（取消标记，后台预压同样受制），下次运行恢复正常 |
| 中途压缩被中止信号打断 | 同取消；运行本身走中止路径 |
| 运行被用户中止时在飞的后台预压 | 不随之中止：照常跑完并写入元数据（下次运行受益）；daemon 退出时被掐断，等于没发生 |
...
| 请求真超限（前面所有手段失守） | 识别上下文超限错误 → 掐掉在飞后台压缩 → 紧急压缩 → 整次请求静默重发一次；重发仍超限才把错误报给用户 |
| 收尾压缩期间用户发消息 | 会话保持忙碌，新消息走既有排队路径，压缩完成后自动开跑 |
| 会话运行中调 /compact | 挂起：返回"已排队：当前运行结束后自动压缩"，运行结束收尾链先冲刷它再做自动收尾压缩 |
...
- [run-manager](../server/run-manager.md)：`Compactor` 的五触发点编排（收尾/中途/后台预压/超限在 run 装配、手动在 compactSession）、`cancelCompaction`、收尾压缩的串行化、/compact 的挂起与冲刷
