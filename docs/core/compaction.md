...
|--------|-----------|------------------------|--------------|--------|
| 后台预压 | 迭代边界：水位进入 [预压线, 红线) 区间且无在飞压缩、无挂起成果 | `in-run` | `in-run`（审计页显示"自动（运行中）"） | 不阻塞（后台进行） |
...
自动压缩可以取消：WebUI 压缩指示行的"取消"按钮发 WS 帧 `compaction.cancel`，服务端中止在飞的摘要调用（含后台预压），并写一个会话级取消标记——本次运行内后续的中途、收尾与后台压缩都被这个标记压制，下一次运行开始时标记清除、恢复正常压缩（服务端细节见 [run-manager](../server/run-manager.md) 的 `cancelCompaction`）。运行被用户中止时在飞的后台压缩**不**随之中止：它的成果照常写入元数据，下次运行直接受益；daemon 退出时它被掐断，等于没发生。手动 /compact 不检查取消标记、与自动压缩互不干涉，WebUI 对手动阶段也不渲染取消按钮（`cancelCompaction` 不作用于手动压缩）。
...
**完成与生效（双重交接）**：后台压缩完成时先把成果无条件写入——`compaction` 事件、`meta.compaction` 投影、总线事件全部落定；随后把新视图**挂起**在内存里（`takeParked` 可取）。应用发生在下一次迭代边界：`mid-run-panic` 一进来先取挂起视图，有就直接采用为新压缩视图（挂起成立说明水位刚过预压线时压的，现在大概率已落回线下，不再压第二次）——当前运行的下一次请求立刻变小。挂起视图只在内存：daemon 重启会丢掉挂起本身，但元数据已经落盘，下次运行从 meta 读到同样的压缩视图，无损失。
...
- **已有排队消息时仍拒绝**：会话不在运行、但队列里已有等待消息时返回 409——排队消息会连开多个 run，压缩窗口无法预期，保持显式拒绝（"还有 N 条排队消息，请稍后再试"）。
...
| 会话有排队消息时调 /compact | 拒绝，返回"还有 N 条排队消息，请稍后再试" |
