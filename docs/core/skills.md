...
- **档位缺省从源技能推断**：建链接不传 `tier` 时，按源 SKILL.md 的 frontmatter 可见性反推（`suggestTier`）——`disable-model-invocation: true` 且 `user-invocable: true` 的技能推断为 `user` 档（仅用户可点名）、只设 `user-invocable: true` 的推断为 `all`、`disable-model-invocation: true` 且 `user-invocable: false`（两面都隐藏）的推断为 `off` 档、其余为 `model` 档；frontmatter 无法解析时回落 `all`。复用继承作者的可见性意图，用户显式选档位才覆盖。
