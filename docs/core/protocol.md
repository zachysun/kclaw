...
持久化单位。信封五字段：`id` / `sessionId` / `role` / `blocks` / `createdAt`（ISO-8601）；按 role 扩展出两个子形状——`AssistantMessage` 补 `model` / `usage` / `stopReason` / 可选 `latencyMs`（LLM 生成耗时，仅流成功完成时存在），`ToolMessage` 补 `grantedBy`（callId → 放行原因的映射）。`Role` 三值、`StopReason` 七值、`GrantedBy` 八值与构造函数（`newMessage` / `newToolMessage` / `newAssistantMessage`）的逐值陈列见 [reference/messages](../reference/messages.md)（含 provider finish_reason 的归一化映射表）。
