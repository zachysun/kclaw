...
- **`next()` 不含边界时刻**：cron-parser 的 `next()` 只返回严格晚于 `currentDate` 的时刻（恰好相等也不算），所以 `nextIsoAfter` 的语义是"严格晚于给定时刻的第一次触发"。无效 cron 表达式原样抛出 cron-parser 自身的错误消息，不做包装。
...
export interface Job {
  ...
  model?: string      // 可选：该 job 的模型覆盖（空/默认 = daemon 默认模型）
  enabled: boolean
...
1. **认领并触发到期的 job**：`scheduler.claimDue(now())` 在一个事务内认领到期集合，认领即把每行的 `nextRunAt` 当场推进到 now 之后的下一次（CAS 更新，恰好命中一行才算认领成功），然后逐个（同一轮内串行）触发不在 `inFlight` 集合里的 job。
...
4. `run.enqueue(sessionId, { userText: job.prompt, trigger: "job", note: "本会话由定时任务「<name>」触发", ...(job.model !== undefined && job.model !== "" ? { model: job.model } : {}) })`——RunManager 把 prompt 作为用户消息发起一次运行（job 配置的 `model` 作为本次运行的模型覆盖透传，默认则走会话/默认解析链），该 note 写入用户消息上紧跟正文的 `kind: "job"` note 块（组装过程见 core `packages/core/src/agent/run-assembly.ts` 的 `executeRun`）。
