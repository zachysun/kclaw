...
7. **新增一个技能包（不改代码）**：一个技能是一个目录，里面放一份 `SKILL.md`（YAML 头部 + Markdown 正文）。放进 `~/.kclaw/skills/<目录名>/`（全局）或工作区 `.kclaw/skills/<目录名>/`（项目级，覆盖同名全局技能），下一个会话轮次即生效——daemon 每个 run 重新扫描目录，模型经 `skill_read` 工具按名加载正文（机制与五个可解释字段见 [skills](./core/skills.md)）。想从模型可见列表里藏起来就加 `disable-model-invocation: true`，想彻底从用户界面消失（斜杠命令/技能页都不可见）就加 `user-invocable: false`。
