...
9. **模型解析与 LLM 客户端**：模型行先解析——`resolveRunModel(config, input.model ?? sessionMeta?.model ?? defaultModel)` 一次得出四件事：线上模型 id（名字命中 provider 条目时取条目的 `model`，未命中则原样是裸模型名）、命中的条目名 `entryKey`（未命中回落默认条目）、本 run 的上下文预算（条目 `contextWindow` 与会话预算取 min）、条目声明的 `maxOutput`。随后 `llmForRun(onLlmRetry, entryKey)` 为本 run 构造带重试可见性的客户端——重试回调把 attempt 计数推进并触发 `llm-retry` 位置的钩子链（内置 `retry-notify` 转 `llm.failed {willRetry:true}` 事件）；daemon 侧按 `entryKey` 取该条目自己的端点与协议客户端（按签名缓存，改配置下个 run 热生效，见 [provider](../core/provider.md)）；`runId` 从循环的第一个事件 `run.started` 捕获，`llmAttempt` 计数在 `llm.completed/failed` 后复位。
