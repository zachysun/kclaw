...
`enabled` 缺省 false；开启后 `app_id` 与 `app_secret` 必填，否则 daemon 启动时报一行错误并跳过频道（其余功能不受影响）。`allowlist` 是 open_id 白名单，白名单外的发件人被**静默忽略**（记一行日志，不回任何信息）——应用被拉进陌生群或被陌生人私聊都不会泄露内容。`primaryOpenId` 接收主动推送（定时任务终态、后台子代理完成），它必须同时出现在 `allowlist` 里，否则不推送。密钥独立于 config.yaml，是为了不让 app_secret 混进主配置的备份与同步路径。
...
**启动时机与失败行为**：daemon 在任务调度与记忆调度都起转之后才启动飞书频道，且启动有 15 秒上限（`FEISHU_START_TIMEOUT_MS`）——飞书的 WebSocket 握手在坏网络（DNS、代理、防火墙故障）下可能无限挂起，超时即放弃这次启动：报一行错误、拆掉半启动的连接与总线订阅，daemon 其余功能照常。真传输的握手本身另有 10 秒死线，多数坏网络在这一步就快速失败，到不了 15 秒。官方 SDK 只在频道启用时才加载，未启用飞书的 daemon 不为它付任何启动成本。
