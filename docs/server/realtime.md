...
| `{"type":"send_message","sessionId","text","attachments"?,"disposition"?,"target"?}` | sessionId/text 非空字符串；`disposition` 可选 `"steer"\|"wait"\|"interrupt"`，缺省 = 会话覆盖（`SessionMeta.dispositionOverride`）?? 配置 `sessions.defaultDisposition` ?? steer；`attachments` 可选，为 `[{path,name,size,mimeType}]` 数组——path 经 realpath 校验必须位于本会话的附件目录（`<attachmentsDir>/<sessionId>/`）内，否则整条拒绝（任意路径会让持 token 者读到 daemon 可达的任意文件）；`target` 可选非空字符串——agent 团队的组员名，带它时这条消息**不经会话自身的 run**，改走该组员的收信箱投递（同时落进组长历史附转发 note，机制见 [agent-team](../core/agent-team.md)），ack 恒 `queued:false` | `{"type":"send_message_ack","sessionId","messageId","queued"}`——**立即**返回，不等 run；`messageId` 是入队时预分配的消息 id（后续事件与 JSONL 都用它，气泡原地升级的锚点）；`queued:false` = 会话空闲直发（不广播 `message.queued`） | 同步失败（无 ack）：`session not found`；`队列已满（10 条）`；`send_message disposition must be "steer", "wait" or "interrupt"`；`send_message attachments are invalid`；`send_message target must be a non-empty string (a member name)`；团队投递路径下 `no active member named "<target>"` 与 `team host not available`（daemon 未装配团队宿主）；`run manager not available`；字段不合法提示。**ack 之后不发迟到 error 帧**（`submit().outcome` 发出后不等待结果）：run 级失败经 `run.failed`、条目级失败经 `run.failed {error.code:"queue_entry_failed"}` 到达订阅者（见下文"消息排队与引导"） |
| `{"type":"message.retry","sessionId","fromMessageId","text","attachments"?}` | sessionId/fromMessageId 非空字符串；text 是**字符串**（允许为空——纯附件重试，无文本且无附件的组合由 run 管理器拒绝，它才知道被重做的回合是否带附件）；`attachments` 可选、校验同 send_message（必须在本会话附件目录内）；`fromMessageId` 是否最后一条 user 消息由 run 管理器验证（command-check 只做字段形状检查） | `{"type":"message.retry_ack","sessionId","messageId","queued"}`——同步决策（先写截断标记 + 广播 `message.truncated`，再走普通用户消息路径提交），形状与 send_message_ack 一致 | 同步失败（无 ack）：`session not found`；`message.retry requires a non-empty string sessionId, a non-empty string fromMessageId and a string text`；`message.retry attachments are invalid`；`只能从最后一条用户消息重试`；`会话忙：等当前运行和压缩结束、清空队列后再重试`；`重试内容为空（无文本也无附件）`；子代理会话只读拒绝；`run manager not available`。编辑重试/重新生成的服务端机制见 [run-manager](./run-manager.md) |
...
- [protocol](../core/protocol.md)：40 种事件与 payload 全表、信封字段
