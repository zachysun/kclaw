...
- **daemon 就绪信号**：`launchDaemon` 装配完成、listen 成功后写 `<home>/daemon.json` `{port, pid, startedAt}`；bin 脚本向 stdout 打一行 `{"port":<port>}`。
...
- **已知限制**：glob 白名单不防御 shell 注入（`exec:git diff*` 也匹配 `git diff; curl evil`，敏感场景依赖 confirm 人工确认）；fs 工具软沙箱是字面（lexical）路径检查、不追 symlink；CLI 的 respawn 目标解析假定 repo checkout（`packages/cli` 与 `packages/server` 相邻）——独立分发包由 `kclaw` 包的 esbuild 产物解决。
